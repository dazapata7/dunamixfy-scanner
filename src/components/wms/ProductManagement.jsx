...
const TYPE_META = {
  ...
  combo:  { label: 'Compuesto', color: 'bg-blue-500/10 border-blue-500/20 text-blue-400/80' },
};
...
export function ProductManagement() {
  ...
  async function loadProducts() {
    ...
    try {
      ...
      setProducts(data.filter(p => ['simple', 'combo'].includes(p.type)));
  ...
  async function openEdit(product) {
    ...
    try {
      const mappings = await skuMappingsService.getByProductId(product.id);
  ...
  const compuestos = filtered.filter(p => p.type === 'combo');
            ...
            {filtered.length === 0 ? (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-white/[0.03]">
                  <TableSection title="Simples" items={simples} color="text-primary-400/60" />
                  <TableSection title="Compuestos" items={compuestos} color="text-blue-400/60" />
                </tbody>
              </table>
            )}
          </div>
          ...
          <div className="lg:hidden space-y-4">
            {filtered.length === 0 ? (
              ...
              <>
                {simples.length > 0 && (
                  <div>
                    <p className="text-white/25 text-[11px] uppercase tracking-[0.12em] mb-2 px-1">Simples ({simples.length})</p>
                    <div className="space-y-2">{simples.map(p => <ProductCard key={p.id} product={p} />)}</div>
                  </div>
                )}
                {compuestos.length > 0 && (
                  <div>
                    <p className="text-white/25 text-[11px] uppercase tracking-[0.12em] mb-2 px-1">Compuestos ({compuestos.length})</p>
                    <div className="space-y-2">{compuestos.map(p => <ProductCard key={p.id} product={p} />)}</div>
                  </div>
                )}
              </>
            )}
