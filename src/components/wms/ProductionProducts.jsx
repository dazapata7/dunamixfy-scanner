...
import { productsService, skuMappingsService, categoriesService, bomService } from '../../services/wmsService';
...
export function ProductionProducts() {
  ...
  async function loadProducts() {
    ...
    try {
      ...
      setProducts(data.filter(p => PRODUCTION_TYPES.includes(p.type)));
      setSimpleProducts(data.filter(p => ['simple', 'combo'].includes(p.type)));
  ...
  async function handleSave() {
    ...
    try {
      ...
      delete payload.linked_product_id; // TODO: save linked_product_id when DB column exists
  ...
  const TableSection = ({ title, items, color }) => (
    items.length === 0 ? null : (
      <>
        <tr><td colSpan={6} className="px-4 pt-5 pb-2"><p className={`text-[10px] font-semibold uppercase tracking-widest ${color}`}>{title} ({items.length})</p></td></tr>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {['finished_good', 'semi_finished'].includes(formData.type) && (
                  <div className="md:col-span-2">
                    ...
                    <p className="text-white/20 text-xs mt-1">El inventario de producción se asignará a este producto</p>
                  </div>
                )}
