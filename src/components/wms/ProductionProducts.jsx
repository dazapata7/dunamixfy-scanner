...
import { productsService, skuMappingsService, categoriesService, bomService, inventoryService } from '../../services/wmsService';
...
import { buildCapacityContext, enrichWithCapacity } from '../../utils/productionCapacity';
...
export function ProductionProducts() {
  ...
  async function loadProducts() {
    setIsLoading(true);
    try {
      const [data, stockRows, reservedRows, bomList] = await Promise.all([
      ...
      // Capacity context (recursivo + memoizado)
      const ctx = buildCapacityContext({ bomList, stockRows, reservedRows });
      const enrichedAll = enrichWithCapacity(data, ctx).map(p => ({
        ...p,
        // Alias compatibilidad con código previo
        stock_in_warehouse: p.stock_fisico,
        linked_name: p.linked_product_id
          ? data.find(x => x.id === p.linked_product_id)?.name || null
          : null,
      }));

      setProducts(enrichedAll.filter(p => PRODUCTION_TYPES.includes(p.type)));
      setSimpleProducts(enrichedAll.filter(p => ['simple', 'combo'].includes(p.type)));
    } catch (err) {
      console.error('[ProductionProducts] loadProducts error:', err);
      toast.error('Error al cargar productos de producción');
    }
  ...
  const canTransfer = (p) =>
    ...
    && p.linked_product_id
    && (p.stock_fisico ?? p.stock_in_warehouse ?? 0) > 0;
  ...
  const ProductRow = ({ product }) => (
    <tr className="hover:bg-primary-500/[0.03] transition-colors group">
      ...
      <td className="px-4 py-3">
        <span className="text-white/70 text-sm font-semibold" title="Unidades ya armadas/ensambladas">
          {product.stock_fisico ?? 0}
        </span>
      </td>
      <td className="px-4 py-3">
        <span
          className={`text-sm font-semibold ${(product.stock_producible ?? 0) > 0 ? 'text-emerald-400/80' : 'text-white/30'}`}
          title="Unidades que podrían fabricarse con los insumos disponibles (cálculo recursivo, optimista)"
        >
          {product.stock_producible ?? 0}
        </span>
      </td>
      <td className="px-4 py-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-2">
          ...
          <span className="text-white/40 text-xs ml-1">
            ...
            Producible: <strong className={(product.stock_producible ?? 0) > 0 ? 'text-emerald-400/80' : 'text-white/40'}>{product.stock_producible ?? 0}</strong>
          </span>
  ...
  const TableSection = ({ title, items, color }) => (
    items.length === 0 ? null : (
      <>
        <tr><td colSpan={9} className="px-4 pt-5 pb-2"><p className={`text-[10px] font-semibold uppercase tracking-widest ${color}`}>{title} ({items.length})</p></td></tr>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/[0.05] bg-black/20">
                    ...
                    <th className="px-4 py-3 text-left text-white/25 font-medium text-[11px] uppercase tracking-[0.12em] w-20" title="Unidades ya armadas (en inventario)">Físico</th>
                    <th className="px-4 py-3 text-left text-white/25 font-medium text-[11px] uppercase tracking-[0.12em] w-24" title="Unidades fabricables con insumos disponibles">Producible</th>
